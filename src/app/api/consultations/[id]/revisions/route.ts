...
import { saveClinicalNoteRevision } from "@/modules/consultations/service";
...
export async function POST(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const actor = await requireCapability("clinical_notes.write", request.headers);
    const id = consultationIdSchema.parse((await context.params).id);
    const result = await saveClinicalNoteRevision(
      id,
