...
import { updatePendingFollowUp } from "@/modules/follow-ups/service";
...
export async function POST(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const actor = await requireCapability("followups.update", request.headers);
    const id = followUpIdSchema.parse((await context.params).id);
    const updated = await updatePendingFollowUp(id, await request.json().catch(() => null), actor);
