...
import { voidPrescription } from "@/modules/prescriptions/service";
...
export async function POST(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const actor = await requireCapability("prescriptions.void", request.headers);
    const id = prescriptionIdSchema.parse((await context.params).id);
    return prescriptionJson({
      prescription: await voidPrescription(id, await request.json().catch(() => null), actor),
    });
  } catch (error) {
