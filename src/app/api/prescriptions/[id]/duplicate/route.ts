...
import { duplicatePrescription } from "@/modules/prescriptions/service";
...
export async function POST(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const actor = await requireCapability("prescriptions.duplicate", request.headers);
    const id = prescriptionIdSchema.parse((await context.params).id);
    return prescriptionJson(
      { prescription: await duplicatePrescription(id, actor) },
      { status: 201 },
    );
  } catch (error) {
