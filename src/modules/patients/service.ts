...
export async function changePatientArchiveState(
  ...
  return db.transaction(async (transaction) => {
    ...
    if (archive) {
      ...
      const [openConsultation] = await transaction
        .select({ id: consultation.id })
        .from(consultation)
        .where(and(eq(consultation.patientId, patientId), eq(consultation.status, "IN_PROGRESS")))
        .limit(1);
      if (openConsultation) {
        throw new ConflictError(
          "Finalize the patient's in-progress consultations before archiving the record.",
        );
      }

      const [pendingFollowUp] = await transaction
        .select({ id: followUp.id })
        .from(followUp)
        .where(and(eq(followUp.patientId, patientId), eq(followUp.status, "PENDING")))
        .limit(1);
      if (pendingFollowUp) {
        throw new ConflictError(
